...
- **Stations** → 5 project stations (the real deployed roster shared with jacinto.design), each **kit-bashed into its own facility** from the dish / cargo / satellite props — a mail relay, a survey constellation, a sound dock, a rogue's den, a galley convoy — with a Japanese blade-sign, a billboarded neon nameplate, and a bracketed **holo projection** (scanlines, orbital arcs, emitter node) showing a real screenshot or demo clip of the work
...
- **Station kit-bash** — per-project prop arrangements live in `src/stations/Station.tsx` (`KITS`)
